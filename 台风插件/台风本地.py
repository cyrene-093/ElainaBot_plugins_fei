...
log = get_logger(PLUGIN, '台风本地')

NMC = 'https://typhoon.nmc.cn/weatherservice/typhoon/jsons'
DATA_DIR = os.path.join(os.path.dirname(__file__), 'data')
os.makedirs(DATA_DIR, exist_ok=True)
DB_PATH = os.path.join(DATA_DIR, 'taifeng_local.db')
...
def _power_of(level, wind=None):
    if level:
        p = _LEVEL_POWER.get(str(level).strip())
        if p:
            return p
    try:
        ws = float(wind)
...
def parse_jsonp(text):
    """中央气象台返回 JSONP：callback({...}) 或 callback(({...}))"""
    text = (text or '').strip()
...
def _level_label(level):
    ...
    return _LEVEL_CN.get(level, level or '-')


async def http_bytes(url, timeout=10, headers=None):
    try:
...
async def http_text(url, timeout=12, headers=None):
    try:
        ...
        async with session.get(url, timeout=to, headers=hdrs) as resp:
            if resp.status != 200:
                log.warning('NMC HTTP %s %s', resp.status, url)
                return None
            return await resp.text()
    except Exception as e:
        log.warning('NMC 请求失败 %s: %s', url, e)
        return None


async def nmc_list(year=None):
    """list_default 或 list_YYYY"""
    ...
    cb = f'typhoon_jsons_list_{key}'
    url = f'{NMC}/list_{key}?t={t}&callback={cb}'
    ...
    for item in data.get('typhoonList') or []:
        if not isinstance(item, (list, tuple)) or len(item) < 8:
            continue
        cn, en = _clean_name(item[2]), _clean_name(item[1])
        rows.append({
            'id': item[0],
            'en': en,
            'cn': cn,
...
async def nmc_view(tid):
    ...
    if cached is not None:
        return cached
    t = int(time.time() * 1000)
    cb = f'typhoon_jsons_view_{tid}'
    url = f'{NMC}/view_{tid}?t={t}&callback={cb}'
    ...
    for p in ty[8] or []:
        ...
        points.append({
            ...
            'radius7q': _quad_radii(wr, '30KTS'),
            'radius10q': _quad_radii(wr, '50KTS'),
            'radius12q': _quad_radii(wr, '64KTS'),
            'forecasts': forecasts,
        })
    result = {
        'id': ty[0],
...
async def resolve_id(keyword):
    ...
    return None, '未找到该台风。可试：台风本地查询 沙德尔 / 2618'
...
def fmt_list_active(rows):
    ...
    for it in active:
        link = _inline(f'台风本地查询 {it["id"]}', it['cn'] or it['en'] or str(it['id']))
        en = f'（{it["en"]}）' if it.get('en') else ''
        md += f'- {link}{en}`{it["num"]}`\n'
    md += f'\n历史：{_inline("台风本地列表 2024")} / {_inline("台风本地年份")}'
    return md
...
def fmt_year(bundle):
    ...
    for it in rows[:30]:
        ...
        link = _inline(f'台风本地查询 {it["id"]}', it['cn'] or it['en'] or '未命名')
        en = f' {it["en"]}' if it.get('en') else ''
...
def _collect_track(view):
    """只取最新实况点上的预报，避免停编台风仍显示过期「未来」路径。"""
    hist = []
    for p in view.get('points') or []:
        lat, lng = _to_float(p.get('lat')), _to_float(p.get('lng'))
        if lat is not None and lng is not None:
            hist.append((lng, lat, p))
    forecast = []
    # 停编台风忽略残留预报，避免显示过期「未来」
    if hist and view.get('status') != 'stop':
        for fp in hist[-1][2].get('forecasts') or []:
            lat, lng = _to_float(fp.get('lat')), _to_float(fp.get('lng'))
            if lat is not None and lng is not None:
                forecast.append((lng, lat, fp))
...
def _pick_marks(hist, forecast, view=None):
    ...
    if hist:
        n = len(hist) - 1
        idxs = {0, n, max(range(len(hist)), key=lambda i: _to_float(hist[i][2].get('wind')) or 0)}
        idxs.update(round(k * n / 6) for k in range(1, 6))
        marks = [('实况', *hist[i]) for i in sorted(idxs)]
    for lng, lat, p in forecast:
        try:
            have[int(p.get('hour'))] = (lng, lat, p)
        except (TypeError, ValueError):
            pass
    marks += [('预报', *have[h]) for h in (6, 12, 18, 24, 36, 48, 72, 96) if h in have]
    out = []
    for m in marks:
        if any(abs(m[1] - x) < 0.4 and abs(m[2] - y) < 0.4 for _, x, y, _ in out):
            continue
        if view:
            lo, hi, la, ha = view
...
def _fit_view(hist, forecast):
    """按近期实况+预报收紧视野，略放大边距以便看清陆域。"""
    lo, hi, la, ha = _VIEW_MAX
    recent = hist[-max(12, len(hist) // 3):] if hist else []
    pts = recent + forecast or (hist[-8:] if hist else [])
    if not pts:
        return _VIEW_MAX
...
def _draw_callout(draw, x, y, lines, font, bounds, occupied, path_pts=None):
    """箭头气泡：指向点旁空白处，避开路径与其它标注。"""
    gap = 1
    ...
    dirs = (
        ...
        (1.2, -0.35), (-1.2, -0.35), (1.2, 0.35), (-1.2, 0.35),
    )
    best = None
    for dist in (36, 52, 68, 86, 104, 122):
        for dx, dy in dirs:
            L = math.hypot(dx, dy) or 1
            ux, uy = dx / L, dy / L
            bx0 = int(x + ux * dist - bw / 2)
            by0 = int(y + uy * dist - bh / 2)
            bx0 = min(max(bx0, mx0 + 4), mx1 - bw - 4)
            by0 = min(max(by0, my0 + 4), my1 - bh - 4)
            box = (bx0, by0, bx0 + bw, by0 + bh)
            if abs((bx0 + bw / 2) - x) < 18 and abs((by0 + bh / 2) - y) < 18:
                continue
            if any(_hit(box, o, 8) for o in occupied):
                continue
            if _box_hits_pts(box, path_pts, 14):
                continue
            cxb, cyb = bx0 + bw / 2, by0 + bh / 2
            mind = min((math.hypot(px - cxb, py - cyb) for px, py in path_pts), default=100)
            score = mind * 1000 - abs(dist - 60) * 2
    ...
    # 气泡
    draw.rectangle((bx0, by0, bx1, by1), fill=(255, 255, 255), outline=(70, 90, 110))
    # 箭头：从气泡边中点指向目标
    tcx, tcy = (bx0 + bx1) / 2, (by0 + by1) / 2
    if abs(x - tcx) > abs(y - tcy):
        ax = bx1 if x > tcx else bx0
        ay = min(max(y, by0 + 4), by1 - 4)
    else:
        ax = min(max(x, bx0 + 4), bx1 - 4)
        ay = by1 if y > tcy else by0
    vx, vy = x - ax, y - ay
    L = math.hypot(vx, vy) or 1
    ux, uy = vx / L, vy / L
    tip = (x - ux * 10, y - uy * 10)
    ox, oy = -uy * 5, ux * 5
    draw.line([(ax, ay), tip], fill=(70, 90, 110), width=1)
    draw.polygon(
        [tip, (tip[0] - ux * 8 + ox, tip[1] - uy * 8 + oy),
         (tip[0] - ux * 8 - ox, tip[1] - uy * 8 - oy)],
        fill=(70, 90, 110),
    )
    ty = by0 + 4
    for t, (_, h) in zip(lines, sizes):
        draw.text((bx0 + 5, ty), t, font=font, fill=(20, 30, 50))
        ty += h + gap
    occupied.append((bx0, by0, bx1, by1))
    return (bx0, by0, bx1, by1)


def _draw_info_panel(draw, lines, font, bounds, occupied, path_pts, prefer='tr'):
    """把当前基本信息画在角落空白处。"""
    gap = 1
    sizes = [_tw(draw, t, font) for t in lines]
    ...
    for key in order:
        ...
        bx0, by0 = corners[key]
        box = (bx0, by0, bx0 + bw, by0 + bh)
        if any(_hit(box, o, 6) for o in occupied):
            continue
        if _box_hits_pts(box, path_pts, 10):
            continue
        draw.rectangle(box, fill=(255, 255, 255), outline=(50, 70, 100), width=2)
        draw.rectangle((bx0, by0, bx0 + 4, by0 + bh), fill=(30, 100, 180))
        ty = by0 + 6
        for t, (_, h) in zip(lines, sizes):
            draw.text((bx0 + 10, ty), t, font=font, fill=(20, 30, 50))
            ty += h + gap
        occupied.append(box)
        return box
    # 兜底：右上
...
# 兼容旧名
def _draw_path_text(draw, x, y, lines, font, side, bounds, occupied):
    return _draw_callout(draw, x, y, lines, font, bounds, occupied)
...
def render_track_png(view):
    ...
    W, H, HEAD = 900, 720, 56
    mx0, my0, mw, mh = 24, HEAD, W - 48, H - HEAD - 16
    img = Image.new('RGB', (W, H), (255, 255, 255))
    d = ImageDraw.Draw(img)
    ft, fs, fxs = _font(22), _font(13), _font(11)
    ...
    if fs:
        sw, _ = _tw(d, sub, fs)
        d.text(((W - sw) / 2, 34), sub, font=fs, fill=(80, 80, 80))
    ...
    def _vis(ring):
        xs, ys = [a for a, _ in ring], [b for _, b in ring]
        return not (max(xs) < min_lng or min(xs) > max_lng or max(ys) < min_lat or min(ys) > max_lat)

    for ring in _LAND_POLYS:
        if _vis(ring):
            # 大陆块留给省界填充，避免粗海岸盖住真省界
            xs = [a for a, _ in ring]
            if max(xs) - min(xs) > 30:
                continue
            pts = [lp(*p) for p in ring]
            if len(pts) >= 3:
                ld.polygon(pts, fill=(232, 220, 190), outline=(150, 140, 120))
    for ring in _PROV_LINES:
        if _vis(ring):
            pts = [lp(*p) for p in ring]
            if len(pts) >= 3:
                ld.polygon(pts, fill=(232, 220, 190), outline=(70, 60, 50))
    for i in range(5):
        x, y = mw * i / 4, mh * i / 4
    ...
    for i in range(5):
        ...
        if fxs:
            d.text((x - 12, my0 + mh - 14), f'{min_lng + (max_lng - min_lng) * i / 4:.0f}°E', font=fxs, fill=(70, 90, 110))
            d.text((mx0 + 3, y - 6), f'{max_lat - (max_lat - min_lat) * i / 4:.0f}°N', font=fxs, fill=(70, 90, 110))
    ...
    if fs:
        for lng, lat, name in _CN_LABELS:
            ...
            for ox, oy in ((-1, 0), (1, 0), (0, -1), (0, 1)):
                d.text((x + 3 + ox, y - 5 + oy), name, font=fs, fill=(255, 255, 255))
            d.text((x + 3, y - 5), name, font=fs, fill=(40, 55, 80))
    ...
    if fxs:
        ...
        for name, col, st in (
            ...
            _draw_dot(d, lx + 6, ly + 5, col, st)
            d.text((lx + 16, ly - 1), name, font=fxs, fill=(40, 40, 40))
            ly += 16
        if alive and any(cur.get(k) for k in ('radius7', 'radius10', 'radius12')):
            for name, col in (('7级风圈', (255, 215, 0)), ('10级风圈', (255, 140, 0)), ('12级风圈', (230, 40, 40))):
                d.rectangle((lx, ly, lx + 12, ly + 10), fill=col, outline=(60, 60, 60))
                d.text((lx + 16, ly - 1), name, font=fxs, fill=(40, 40, 40))
                ly += 16
        occupied.append((mx0 + 6, my0 + 6, mx0 + 100, ly + 4))
    ...
    if alive:
        rparts = [f'{lab}{cur.get(k)}km' for lab, k in (('7级', 'radius7'), ('10级', 'radius10'), ('12级', 'radius12')) if cur.get(k)]
        if rparts:
            info_lines.append('风圈 ' + ' / '.join(rparts))
    prefer = 'tl' if cx > mx0 + mw * 0.5 else 'tr'
    panel = _draw_info_panel(d, info_lines, fxs or fs, bounds, occupied, path_pts, prefer=prefer)
    if panel:
        d.line([((panel[0] + panel[2]) / 2, panel[3] if cy > panel[3] else panel[1]), (cx, cy)], fill=(30, 100, 180), width=1)
    ...
    for kind, lng, lat, p in marks:
        ...
        if strong:
            lines.append(strong)
        _draw_callout(d, *proj(lng, lat), lines, fxs or fs, bounds, occupied, path_pts)
...
async def upload_track_image(event, png, filename='typhoon_track.png'):
    try:
        from core.application import get_app
        app = get_app()
        ...
        return await hosting.upload_any(png, filename, token_manager=getattr(bot, 'token_manager', None) if bot else None)
    except Exception as e:
        log.warning('台风图床上传失败: %s', e)
        return None


def fmt_detail(view, *, active_meta=None, image_md=''):
    ...
    lines.append(f'参考：近{_near_city(_to_float(p.get("lng")) or 0, _to_float(p.get("lat")) or 0)}')
    fc = (p.get('forecasts') or []) if view.get('status') != 'stop' else []
    if fc:
...
async def reply_detail(event, view, buttons, *, active_meta=None, t0=None):
    png = image_md = None
    try:
        png = await asyncio.to_thread(render_track_png, view)
    except Exception as e:
        log.warning('路径图失败: %s', e)
    if png:
    ...
    md = fmt_detail(view, active_meta=active_meta, image_md=image_md or '') + suffix
    if image_md:
        try:
    ...
    await safe_reply(event, fmt_detail(view, active_meta=active_meta) + suffix, buttons)
...
@handler(r'^\s*/?台风本地\s*$', name='台风本地', desc='当前最强活跃台风', ignore_at_check=True)
@guard
async def cmd_strongest(event, match):
    ...
    await reply_detail(
        ...
        active_meta=best, t0=start,
    )


@handler(r'^\s*/?台风本地(?:列表|活跃)(?:\s+(\d{4}))?\s*$', name='台风本地列表', desc='活跃或按年列表', ignore_at_check=True)
@guard
async def cmd_list(event, match):
    ...
    await safe_reply(event, md, _btns(
        [('刷新', f'台风本地列表 {year}' if year else '台风本地列表'), ('本年', '台风本地年份')],
        [('查询', '台风本地查询 '), ('帮助', '台风本地帮助')],
    ))


@handler(r'^\s*/?台风本地年份(?:\s+(\d{4}))?\s*$', name='台风本地年份', desc='按年列表', ignore_at_check=True)
@guard
async def cmd_year(event, match):
    ...
    await safe_reply(event, md, _btns(
        [('刷新', f'台风本地年份 {year}'), ('活跃', '台风本地列表')],
        [('查询', '台风本地查询 '), ('帮助', '台风本地帮助')],
    ))


@handler(r'^\s*/?台风本地查询\s*(.+?)\s*$', name='台风本地详情', desc='查详情与路径图', ignore_at_check=True)
@guard
async def cmd_detail(event, match):
    start = time.time()
    keyword = (match.group(1) or '').strip()
    if not keyword:
        await safe_reply(event, '请输入名称/编号/ID，例如：台风本地查询 沙德尔')
        return
...
@handler(r'^\s*/?台风本地帮助\s*$', name='台风本地帮助', desc='帮助', ignore_at_check=True)
@guard
async def cmd_help(event, match):
    md = (
        '**台风本地图** v1.0.0 · 中央气象台\n\n'
        f'> {_inline("台风本地")} 当前最强活跃台风 + 中央台风格路径图\n'
        f'> {_inline("台风本地列表")} 活跃列表\n'
        f'> {_inline("台风本地列表 2024")} / {_inline("台风本地年份")} 按年\n'
        f'> {_inline("台风本地查询 ", "台风本地查询")} 名称/编号/ID\n'
        f'> {_inline("我的台风本地记录")}\n\n'
        f'示例：{_inline("台风本地查询 沙德尔")} · {_inline("台风本地查询 2618")} · {_inline("台风本地查询 3304099")}\n\n'
        '数据：typhoon.nmc.cn · 中央台风格路径图（嵌入表格）\n'
        '原创：茉莉奶绿 · 修改优化：飞行漂绒'
    )
    await safe_reply(event, md, _btns(
        [('台风本地', '台风本地'), ('列表', '台风本地列表')],
...
@handler(r'^\s*/?我的台风本地记录\s*$', name='我的本地记录', desc='查询历史', ignore_at_check=True)
@guard
