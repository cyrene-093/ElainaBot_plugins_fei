...
def _parse_pub_page(html):
    ...
    if not imgs:
        imgs = re.findall(
            r'(https?://image\.nmc\.cn/product/[^"\'?\s]+TCBU[^"\'?\s]+\.(?:JPG|jpg|PNG|png))',
            html or '',
        )
...
def _rounded_bubble(draw, box, fill, outline, radius=10, width=1):
    """圆角矩形；旧 Pillow 无 rounded_rectangle 时退回直角。"""
    try:
...
def _bubble_tail(draw, box, tx, ty, fill, outline, *, tip_gap=7, base=8):
    ...
    tip = (tx - ux * tip_gap, ty - uy * tip_gap)
    # 选离目标最近的边作为底座
    candidates = (
    ...
    for dist in (40, 56, 72, 90, 110, 130, 150):
        for dx, dy in dirs:
            ...
            if abs((bx0 + bw / 2) - x) < 20 and abs((by0 + bh / 2) - y) < 20:
                continue
            if any(_hit(box, o, 7) for o in occupied):
                continue
            if _box_hits_pts(box, path_pts, 12):
                continue
            # 尖端连线走廊尽量少压路径
...
def _defense_tips(view):
    ...
    if wind >= 24.5 or level in ('TY', 'STY', 'SuperTY', 'SUPERTY') or '台风' in strong:
        tips = [
...
def fmt_detail(view, *, active_meta=None, image_md='', map_note='', map_title='路径图'):
    ...
    block.append(f'状态  {"活跃" if alive else "停编"}')
    block += [
...
async def reply_detail(event, view, buttons, *, active_meta=None, t0=None):
    """优先中央台官网路径产品图，没有则本地绘图。耗时统计含配图阶段。"""
    png = image_md = None
    map_note = ''
    map_title = '路径图'
    filename = 'typhoon_track.png'
    try:
        if view.get('status') == 'stop':
            off, src = None, 'no_official'
        else:
            off, src = await fetch_official_track_png(view)
        if off and isinstance(src, str) and src.startswith('http'):
            png = off
            map_note = '配图：中央气象台官方路径产品图'
            map_title = '中央台官方路径图'
            filename = 'typhoon_nmc.jpg'
    ...
    if png:
        url = await upload_track_image(event, png, filename=filename)
        if url:
            w, h = _png_wh(png)
            image_md = f'![台风路径 #{w}px #{h}px]({url})'
    suffix = f'\n耗时 {_ms(t0)}ms' if t0 is not None else ''
    md = fmt_detail(
        view, active_meta=active_meta, image_md=image_md or '',
        map_note=map_note, map_title=map_title,
    ) + suffix
    if image_md:
        try:
    ...
    await safe_reply(
        event,
        fmt_detail(view, active_meta=active_meta, map_note=map_note, map_title=map_title) + suffix,
        buttons,
    )
...
async def cmd_list(event, match):
    ...
    await safe_reply(event, md, _btns(
        [('刷新', f'台风列表 {year}' if year else '台风列表'), ('本年', '台风年份')],
        [('查询', '台风查询 '), ('帮助', '台风帮助')],
    ))
